...
(function(global){
  ...
  function attachTagRowEvents(scope=document){ scope.querySelectorAll('.tag-row').forEach(tr=>{ tr.addEventListener('click',()=>{ const d=tr.getAttribute('data-device-id'); const t=tr.getAttribute('data-tag-id'); const n=tr.getAttribute('data-tag-name')||''; if(d&&t){ openDrawer(d,t,n); } }); }); }
  ...
  function updateAccuracy(){ const metricBox=id('metricBox'); if(!metricBox) return;
    const metricDefs={
      MAE:{abbr:'MAE',title:'MAE / 平均绝对误差: (1/n) Σ |A-P|'},
      RMSE:{abbr:'RMSE',title:'RMSE / 均方根误差: sqrt((1/n) Σ (A-P)^2)'},
      MAPE:{abbr:'MAPE',title:'MAPE / 平均绝对百分比误差: mean(|A-P|/|A|)*100% '},
      WAPE:{abbr:'WAPE',title:'WAPE / 加权绝对百分比误差: Σ|A-P| / Σ|A| *100%'} };
    function placeholder(extra){ return `预测指标: <small><abbr title='${metricDefs.MAE.title}'>MAE</abbr>=--</small> | <small><abbr title='${metricDefs.RMSE.title}'>RMSE</abbr>=--</small> | <small><abbr title='${metricDefs.MAPE.title}'>MAPE</abbr>=--</small> | <small><abbr title='${metricDefs.WAPE.title}'>WAPE</abbr>=--</small>${extra||''}`; }
    if(!prediction||!prediction.points||!chartMeta||!chartMeta.pts){ metricBox.innerHTML=placeholder(); metricBox.style.display=''; return; }
    const pts=chartMeta.pts; const fpts=prediction.points; if(!pts.length||!fpts.length){ metricBox.innerHTML=placeholder(); metricBox.style.display=''; return; }
    const map=new Map(); for(const p of pts) map.set(p.t,p.y);
    const matched=[]; for(const fp of fpts){ if(map.has(fp.t)) matched.push({pred:fp.y,actual:map.get(fp.t)}); }
    if(!matched.length){ metricBox.innerHTML=placeholder(`<small style='color:var(--color-text-dim);'> (预测点与历史无重叠)</small>`); metricBox.style.display=''; return; }
    // Core errors
    let se=0,ae=0; const absActuals=[]; for(const m of matched){ const a=Math.abs(m.actual); if(a>0) absActuals.push(a); }
    absActuals.sort((a,b)=>a-b); const mid=Math.floor(absActuals.length/2); const medianAbs=absActuals.length? (absActuals.length%2?absActuals[mid]:(absActuals[mid-1]+absActuals[mid])/2):0; let eps=Math.max(1e-6, medianAbs*0.01); // 1% 中位或最小阈值
    let mapeSum=0,mapeCnt=0; let wAbsErr=0,wAbsAct=0; for(const m of matched){ const err=m.actual-m.pred; const a=Math.abs(m.actual); ae+=Math.abs(err); se+=err*err; if(a>=eps){ mapeSum+=Math.abs(err)/a; mapeCnt++; wAbsAct+=a; wAbsErr+=Math.abs(err); } }
    const n=matched.length; const mae=ae/n; const rmse=Math.sqrt(se/n); const mape=mapeCnt? (mapeSum/mapeCnt*100):null; const wape = wAbsAct? (wAbsErr/wAbsAct*100):null;
    const mapeHtml = mape!=null ? ` | <small><abbr title='${metricDefs.MAPE.title}'>MAPE</abbr>=${mape.toFixed(2)}%</small>` : ` | <small><abbr title='${metricDefs.MAPE.title}'>MAPE</abbr>=NA</small>`;
    const wapeHtml = wape!=null ? ` | <small><abbr title='${metricDefs.WAPE.title}'>WAPE</abbr>=${wape.toFixed(2)}%</small>` : ` | <small><abbr title='${metricDefs.WAPE.title}'>WAPE</abbr>=NA</small>`;
    const extra=` <small style='color:var(--color-text-dim);'>|A|<${eps.toExponential(1)} 过滤${matched.length - mapeCnt}点 (MAPE基数=${mapeCnt})</small>`;
    const html=`预测指标: <small>样本=${n}</small> | <small><abbr title='${metricDefs.MAE.title}'>MAE</abbr>=${mae.toFixed(3)}</small> | <small><abbr title='${metricDefs.RMSE.title}'>RMSE</abbr>=${rmse.toFixed(3)}</small>${mapeHtml}${wapeHtml}${extra}`;
    metricBox.innerHTML=html; metricBox.style.display=''; }
